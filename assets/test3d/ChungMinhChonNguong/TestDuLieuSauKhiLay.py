...
plt.title('Phân bố chênh lệch mũi-mắt')
plt.xlabel('Chênh lệch mũi-mắt (mm)')
plt.ylabel('Tần suất')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'histogram_nose_eye_diff.png'), dpi=300)
plt.close()
...
plt.title('Phân bố độ lệch chuẩn vùng mặt')
plt.xlabel('Độ lệch chuẩn vùng mặt (mm)')
plt.ylabel('Tần suất')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'histogram_face_std_dev.png'), dpi=300)
plt.close()

# 3. Biểu đồ scatter plot cho nose_eye_diff
plt.figure(figsize=(10, 6))
...
# 4. Biểu đồ scatter plot cho face_std_dev
plt.figure(figsize=(10, 6))
