...
class AttendanceSpoofingTest:
    def __init__(self, root):
        ...
        self.test_duration = 20  # Tổng thời gian test (5 góc x 4 giây)
        self.frames_per_angle = 50  # Số khung hình mỗi góc
        self.time_per_angle = 4  # Thời gian mỗi góc (giây)
        ...
        self.frame_interval = self.time_per_angle / self.frames_per_angle  # Khoảng thời gian giữa các khung (0.08 giây)
        ...
        self.label_instruction = Label(root, text="Đưa khuôn mặt thật vào camera và nghiêng đầu theo hướng dẫn.", font=("arial", 12))
    ...
    def start_recognition_test(self):
        if not self.test_running:
            ...
            self.text_output.insert(END, "Hãy chuẩn bị nghiêng đầu theo hướng dẫn.\n")
            self.prompt_for_angle()
    ...
    def prompt_for_angle(self):
        if self.current_angle_index < len(self.angle_list):
            ...
            messagebox.showinfo("Chuẩn bị", f"Vui lòng nghiêng đầu để {instruction.lower()} và nhấn OK để bắt đầu chụp.")
        ...
        if faces:
            for face in faces:
                ...
                if results_3d:
                    ...
                    angle_group = current_angle  # Sử dụng góc hiện tại thay vì gom góc
                    ...
                    self.text_output.insert(END, f"Góc {self.angle_instructions_map[current_angle]}, Nhận diện: {'Thật' if is_real else 'Giả'}, Khung: {self.frame_counter}\n")
                else:
                    self.text_output.insert(END, f"Góc {self.angle_instructions_map[current_angle]}, Không đủ dữ liệu 3D, Khung: {self.frame_counter}\n")
        else:
            self.text_output.insert(END, f"Góc {self.angle_instructions_map[current_angle]}, Không phát hiện khuôn mặt, Khung: {self.frame_counter}\n")
